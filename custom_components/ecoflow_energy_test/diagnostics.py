"""Diagnostics support for EcoFlow Energy Test.
...
def _device_diagnostics(coordinator: EcoFlowDeviceCoordinator) -> dict[str, Any]:
    ...
    last_mqtt_rx_age_s: float | None = None
    if coordinator.last_mqtt_rx_ts > 0:
        last_mqtt_rx_age_s = round(now - coordinator.last_mqtt_rx_ts, 1)
    last_cloud_age_s: float | None = None
    if coordinator.last_cloud_ts > 0:
        last_cloud_age_s = round(now - coordinator.last_cloud_ts, 1)
    ...
    diag: dict[str, Any] = {
        ...
        "data_freshness": {
            ...
            "last_mqtt_rx_age_s": last_mqtt_rx_age_s,
            "last_cloud_age_s": last_cloud_age_s,
            ...
            "http_fallback_active": bool(
                ...
                and coordinator.device_type != DEVICE_TYPE_POWERGLOW
            ),
            "app_detail_polling_active": bool(
                coordinator.enhanced_mode
                and coordinator.device_type == DEVICE_TYPE_POWERGLOW
    ...
    if coordinator.device_type in (DEVICE_TYPE_DELTA3, DEVICE_TYPE_STREAM, DEVICE_TYPE_POWERGLOW):
