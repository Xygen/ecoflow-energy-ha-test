"""EcoFlow Energy Test integration for Home Assistant."""
...
async def async_setup_entry(hass: HomeAssistant, entry: EcoFlowConfigEntry) -> bool:
    """Set up EcoFlow Energy Test from a config entry."""
    ...
    _LOGGER.debug(
        "EcoFlow Energy Test: %d device(s) configured (Enhanced: %d, Standard: %d)",
    ...
    for device_info in devices:
        ...
        if not device_type or device_type == DEVICE_TYPE_UNKNOWN:
            ...
            _LOGGER.warning(
                ...
                "available for this model yet. To help add support, switch on the raw "
                "data capture in the integration options and attach a "
                "diagnostics download when reporting the problem - it records what the "
...
async def _async_reload_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    _LOGGER.debug("Config entry updated — reloading EcoFlow Energy Test")
    await hass.config_entries.async_reload(entry.entry_id)


async def async_unload_entry(hass: HomeAssistant, entry: EcoFlowConfigEntry) -> bool:
    """Unload an EcoFlow Energy Test config entry."""
