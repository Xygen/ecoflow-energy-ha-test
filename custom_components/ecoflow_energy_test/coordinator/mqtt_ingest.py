...
from ..ecoflow.parsers.powerglow import parse_powerglow_quota
...
class MqttIngestMixin:
    ...
    def _on_mqtt_message(self, topic: str, payload: bytes) -> None:
        ...
        self._last_mqtt_rx_ts = time.monotonic()
    ...
    def _parse_message(self, topic: str, payload: bytes) -> dict[str, Any] | None:
            ...
            try:
                ...
                if isinstance(quota_map, dict) and quota_map:
                    ...
                    if self.device_type == DEVICE_TYPE_POWERGLOW:
                        parsed = parse_powerglow_quota(quota_map)
                        return parsed if parsed else None
        ...
        if topic.endswith("/quota"):
            try:
                ...
                if self.device_type == DEVICE_TYPE_POWERGLOW:
                    ...
                    if not isinstance(payload_obj, dict):
                        payload_obj = data
                    parsed = parse_powerglow_quota(payload_obj)
                    return parsed if parsed else None
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    if self.device_type == DEVICE_TYPE_SMARTPLUG:
                        ...
                        return parsed if parsed else None
                    if self.device_type == DEVICE_TYPE_POWERGLOW:
                        parsed = parse_powerglow_quota(data)
                        return parsed if parsed else None
            except (json.JSONDecodeError, UnicodeDecodeError):
                pass
            return None
        ...
        if b"\x0a" in payload[:4]:
            try:
                ...
                if self.device_type == DEVICE_TYPE_POWERGLOW:
                    # Direct HF33 protobuf frames are captured above for
                    # diagnostics, but their field layout is not verified.
                    # PowerGlow telemetry is instead read from the associated
                    # PowerOcean consumer reports (matched by ``hrSn``).
                    return None
