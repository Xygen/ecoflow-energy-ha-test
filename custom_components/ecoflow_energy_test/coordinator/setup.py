...
class SetupMixin:
    ...
    async def _setup_app_auth(self, session: Any) -> None:
        ...
        if self.device_type == DEVICE_TYPE_POWERGLOW:
            self._app_api = app_api
            self._powerglow_parent_devices = [
                {
                    "sn": str(device.get("sn", "")),
                    "product_type": str(device.get("product_type", "")),
                    "name": str(device.get("product_name", "PowerOcean")),
                }
                for device in self._entry.data.get(CONF_DEVICES, [])
                if device.get("device_type") == DEVICE_TYPE_POWEROCEAN
                and device.get("sn")
            ]
            await self._discover_powerglow_parent_devices()
            if not self._powerglow_parent_devices:
                _LOGGER.warning(
                    "PowerGlow %s: no PowerOcean parent found; "
                    "MQTT diagnostics remain active but consumer report polling "
                    "cannot start yet and will be retried automatically",
                    self.device_sn,
                )
        ...
        self._mqtt_client = EcoFlowMQTTClient(
            ...
            user_id=user_id,
            wss_mode=True,
            # The existing EnergyStreamSwitch activation frame is confirmed
            # only for PowerOcean. PowerGlow still subscribes to push topics
            # and parses its known reports, but sends no unverified command.
        ...
        if self._mqtt_monitor_started_ts <= 0:
            self._mqtt_monitor_started_ts = time.monotonic()
    ...
    async def _discover_powerglow_parent_devices(self) -> bool:
        """Merge configured and discovered PowerOcean parent candidates."""
        if self._app_api is None:
            return False

        parent_candidates = list(self._powerglow_parent_devices)
        app_devices = await self._app_api.get_device_list()
        ...
        parent_candidates.extend(await self._app_api.get_powerocean_devices())

        # Later discovery sources carry richer metadata and intentionally win
        # over legacy config entries that lack product_type.
        self._powerglow_parent_devices = list({
            parent["sn"]: parent
            for parent in parent_candidates
            if parent.get("sn")
        }.values())
        self._last_powerglow_parent_discovery_ts = time.monotonic()
        return bool(self._powerglow_parent_devices)
