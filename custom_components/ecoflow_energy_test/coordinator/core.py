...
class EcoFlowDeviceCoordinator(
    ...
    def __init__(
        ...
        # Direct PowerGlow app-MQTT frames are not safely decoded yet. Keep
        # the Enhanced connection for diagnostics, while polling the known
        # read-only heating-rod reports through EcoFlow's consumer endpoint.
        poll_interval = (
            None
            if enhanced_mode and self.device_type != DEVICE_TYPE_POWERGLOW
        ...
        self._app_api: AppApiClient | None = None
        self._powerglow_parent_devices: list[dict[str, str]] = []
        self._last_powerglow_parent_discovery_ts: float = 0.0

        self._last_mqtt_ts: float = 0.0
        # Transport activity is tracked separately from successfully parsed
        # telemetry. PowerGlow direct frames are deliberately diagnostics-only
        # until their wire schema is verified, but receiving them proves the
        # WSS connection is alive and must not trigger reconnect loops.
        self._last_mqtt_rx_ts: float = 0.0
        # One-shot observation baseline before the first successfully applied
        # MQTT frame. Reconnects deliberately do not reset it.
        self._mqtt_monitor_started_ts: float = 0.0
        # App-detail polling is a separate freshness source and must not make
        # the MQTT diagnostic sensor claim that telemetry is being received.
        ...
        state_path = hass.config.path(f".storage/ecoflow_energy_test_{self.device_sn}.json")
        ...
        if self.device_type == DEVICE_TYPE_POWEROCEAN:
            ...
            self._power_to_energy = POWERGLOW_POWER_TO_ENERGY
            self._energy_from_api = POWERGLOW_ENERGY_FROM_API
        else:
    ...
    def last_mqtt_ts(self) -> float:
        """Return the timestamp of the last successfully applied MQTT data."""
        return self._last_mqtt_ts

    @property
    def last_mqtt_rx_ts(self) -> float:
        """Return the timestamp of the last received MQTT data frame."""
        return self._last_mqtt_rx_ts

    @property
    def last_cloud_ts(self) -> float:
        """Return the timestamp of the last PowerGlow app-detail data."""
        return self._last_cloud_ts
