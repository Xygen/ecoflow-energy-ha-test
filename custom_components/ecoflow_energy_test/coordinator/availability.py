...
class AvailabilityMixin:
    ...
    def _mqtt_data_age(self) -> float:
        """Return parsed MQTT age, or elapsed initial observation time."""
        reference = (
            self._last_mqtt_ts
            if self._last_mqtt_ts > 0
            else self._mqtt_monitor_started_ts
        )
        if reference <= 0:
            return float("inf")
        return max(0.0, time.monotonic() - reference)

    def _mqtt_transport_age(self) -> float:
        """Return MQTT receive age independently from parser support."""
        reference = max(self._last_mqtt_rx_ts, self._last_mqtt_ts)
        if reference <= 0:
            reference = self._mqtt_monitor_started_ts
        if reference <= 0:
            return float("inf")
        return max(0.0, time.monotonic() - reference)

    def _data_age(self) -> float:
        """Return the freshest successfully parsed MQTT or cloud-data age."""
        reference = max(self._last_mqtt_ts, self._last_cloud_ts)
        if reference > 0:
            return max(0.0, time.monotonic() - reference)
        return self._mqtt_data_age()
    ...
    def _check_stale(self) -> None:
        ...
        age = self._mqtt_data_age()
        transport_age = self._mqtt_transport_age()
        data_age = self._data_age()
        ...
        if self._http_client is not None:
            ...
            if transport_age > stale_threshold_s:
                ...
                if (
                    ...
                    and (
                        ...
                        or self._last_mqtt_rx_ts > 0.0
                        or self._mqtt_monitor_started_ts > 0.0
                    )
                    ...
                    if not self._stale_reactivate_tried:
                        ...
                        self._log_event(
                            "stale_reactivate", f"age={transport_age:.0f}s"
                        )
                        ...
                        self._log_event(
                            "stale_force_reconnect", f"age={transport_age:.0f}s"
                        )
            ...
            # Entity availability follows parsed MQTT/app-detail freshness,
            # not raw transport traffic. A diagnostics-only frame therefore
            # prevents reconnect churn but cannot make stale values look fresh.
            if self._device_available and data_age >= hard_unavailable_s:
                reconnect_attempts = (
                ...
                age_str = "no data since connect" if max(
                    self._last_mqtt_ts, self._last_cloud_ts
                ) <= 0 else f"{data_age:.0f}s"
            ...
            elif data_age <= stale_threshold_s:
                if not self._device_available:
                    _LOGGER.info(
